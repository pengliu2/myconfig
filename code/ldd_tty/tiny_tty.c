...
#define DRIVER_AUTHOR "Peng Liu"
#define DRIVER_DESC "Tiny Pair TTY driver"
...
#define TINY_TTY_MINORS		2	/* only have 2 devices */

#define TINY_TTY_MINOR_START	64
...
static struct tiny_serial *tiny_client;
static struct tiny_serial *tiny_server;
...
static int tiny_open(struct tty_struct *tty, struct file *file)
{
	...
	printk(KERN_DEBUG "the tty index opened is %d\n", index);
	if (index == 0)
	    tiny = tiny_client;
	else if (index == 1)
	    tiny = tiny_server;
	...
	if (tiny == NULL) {
	    printk(KERN_ERR "the tty index %d is not supported\n", index);
	    return -ENODEV;
	}
...
{
	struct tiny_serial *tiny_src = tty->driver_data;
	struct tiny_serial *tiny_dst = NULL;
	struct tty_struct *tty_dst;
	int i;
	int retval = -EINVAL;
	...
	if (tiny_src == tiny_client)
	{
	    tiny_dst = tiny_server;
	}
	else if (tiny_src == tiny_server)
	{
	    tiny_dst = tiny_client;
	}
	else
	{
	    printk("tiny_serial doesn't belong to this module\n");
	}

	tty_dst = tiny_dst->tty;

	for (i = 0; i < count; ++i) {
	    if (tty_dst->low_latency)
		tty_flip_buffer_push(tty_dst);
	    tty_insert_flip_char(tty_dst, buffer[i], TTY_NORMAL);
	    printk("tiny_tty writing %02X", buffer[i]);
	}
	printk("\n");
	tty_flip_buffer_push(tty_dst);
...
static struct uart_driver ttty_driver = {
	.owner			= THIS_MODULE,
	.driver_name		= "serial",
	.dev_name		= "ttty",
	.major			= TTY_MAJOR,
	.minor			= TINY_TTY_MINOR_START,
	.nr			= TINY_TTY_MINORS,
};
#endif /* Peng */
...
static int __init tiny_init(void)
{
	...
	tiny_tty_driver->minor_start = 128;
	...
	*/
	...
	tiny_client = kmalloc(sizeof(struct tiny_serial), GFP_KERNEL);
	if (!tiny_client)
	    goto nomem_clean;

	tiny_server = kmalloc(sizeof(struct tiny_serial), GFP_KERNEL);
	if (!tiny_server)
	    goto nomem_clean;

	init_MUTEX(&tiny_server->sem);
	init_MUTEX(&tiny_client->sem);

	tty_register_device(tiny_tty_driver, 0, NULL); /* the client */
	tty_register_device(tiny_tty_driver, 1, NULL); /* the server */
